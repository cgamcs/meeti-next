...
export default function CommunityMembership({permissions, communityId}: Props) {
  ...
  return (
    <>
      {canJoin && (
        <button
          className="font-bold text-lg w-full rounded-lg lg:w-auto px-5 py-2 text-white cursor-pointer bg-orange-600"
          onClick={() => handleClick}
        >Inscribirme</button>
      )}
      ...
      {canLeave && (
        <button
          className="font-bold text-lg w-full rounded-lg lg:w-auto px-5 py-2 text-white cursor-pointer bg-red-600"
          onClick={() => handleClick}
        >Abanodrar Comunidad</button>
      )}
    </>
  )
}
